use muxfin::api::{MuxerBuilder, MuxerConfig, VideoCodec};
use muxfin::codec::h264::{annexb_to_avcc, extract_avc_config};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // 1) Produce a regular MP4 using muxfin (non-fragmented).
    let muxfin_mp4_path = out_dir.join("muxfin.mp4");
    {
        let file = File::create(&muxfin_mp4_path)?;
    ...
    let muxfin_fmp4_combined_path = out_dir.join("muxfin_fmp4_combined.mp4");
    {
        let mut f = File::create(&muxfin_fmp4_combined_path)?;
        f.write_all(&init)?;
        f.write_all(&seg)?;
        f.flush()?;
    }
    ...
    eprintln!("Wrote: {}", muxfin_mp4_path.display());
    eprintln!("Wrote: {}", muxfin_fmp4_combined_path.display());
